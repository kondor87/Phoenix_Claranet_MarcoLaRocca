...
- Aver ricavato la stringa di connessione a mongoDB cosi formata: `mongodb://[username:password@]host1[:port1][,host2[:port2],...[,hostN[:portN]]][/[database][?options]]`

- Aver verificato se il replica set è in stato attivo, per fare ciò andare sul nodo Primario del DB tramite SSH. Una volta dentro al server lanciare il comando mongo per entrare in una nuova shell di controllo del DB:
...
- Se vogliamo rendere pubblica l'app eseguiamo questi step:
  - Disassociare il target group dall'internal load balancer creato da ElastiBeanstalk eseguendo il Delete listener dal Load Balancer Internal.
  - Modificare sulla Console EB il Load balancer da internal ad internet-Facing ed associare le subnet publiche al nuovo LoadBalancer.
...
L'abilitazione del monitoraggio e notifica della CPU si può effettuare tramite la console di Elastic Beanstalk di seguito le operazioni:
  - Una volta entrati dentro l'environmnet creato andare su Monitoraggio
  - Selezionare il chart con la CPU e cliccare sul simbolo della campana, cosi facendo si aprirà una nuova pagina dove è possibile inserire:
...
1.Recarsi sul pannello dell'environment creato e selezionare configurazione.
2.Andare nel riquadro Capacità e successivamente cliccare su modifica.
3.Nel Trigger di dimensionamento selezionare:
...
I pre-requisiti per eseguirlo correttamente sono questi:
